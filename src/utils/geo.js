...
class Geo {
  ...
  async getGeo() {
    ...
    if (initConfig) {
      ...
      const { geoInfo, position, error: err } = await this.getCoordinate();
      ...
      if (geoInfo) {
        ...
        const { geoInfo: addressInfo, error: err } = await this.getAddressCode(
          location
        );
        console.log('解析地理信息 end:', addressInfo, err);
        return {
          geoInfo: addressInfo,
          error: err,
        };
  ...
  async getCoordinate() {
    switch (this.loactionType) {
      ...
      // break;
      case 2: // h5
        return await this.geoByH5().catch(error => {
          console.log(error);
          return {
            error,
          };
  ...
  geoInitGd() {
    ...
    const gdMapUrl = `https://webapi.amap.com/maps?v=1.4.10&key=${GD_MAP_KEY}&callback=geoGdLoad`;
    const _this = this;
    return new Promise(function(resolve, reject) {
      ...
      console.warn(`初始化高德api ${sum}次`);
      sum++;
      loadJs(gdMapUrl, {
        async: true,
        onload() {
          // 高德api加载完成
          window.geoGdLoad = function() {
      ...
      if (navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          function error(err) {
            ...
            let tip = '';
            if (err.code === 1) {
              tip = 'Error: 用户不允许地理定位！'; // Access is denied
            } else if (err.code === 2) {
              tip = 'Error: 无法获取当前位置！'; // Position is unavailable!
            } else if (err.code === 3) {
