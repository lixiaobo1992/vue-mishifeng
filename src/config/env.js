// ENV
import { debug, storage } from '@dwdjs/utils';
...
const baseEnv = {
  ...
  regBaseUrl: /(\.mishifeng\.com)/i,
  mode: modes[currentMode] || 'prod', // 运行/编译模式 dev prod testing
  stage: 'prod', // 发行版本 local dev beta prod
  apiEnv: 'prod', // api 环境
  spm: '', // spm
  port: location.port,
  host: location.host,
  ...
  isMode(current) {
    const { mode } = this;
    return (
      mode === current || (Array.isArray(current) && current.includes(mode))
    );
  },
  isEnv(current) {
    const { stage } = this;
    return (
      stage === current || (Array.isArray(current) && current.includes(stage))
    );
  },
  ...
  switchApi(apiEnv, apiUrl) {
    if (!apiEnv) return;
    // 切换环境（指切换api）
    if (Object.keys(ENV).includes(apiEnv)) {
